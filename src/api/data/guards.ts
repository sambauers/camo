...
export const isDataRow = (
  maybeDataRow: unknown
): maybeDataRow is Data.Row =>
  Array.isArray(maybeDataRow) &&
  maybeDataRow.every(isDataCell)

export const isSafeData = (
  maybeData: unknown
): maybeData is Data.SafeData =>
  Array.isArray(maybeData) &&
  maybeData.every(isDataRow)
...
export const isUnsafeData = (
  ...
  (
    ...
    maybeUnsafeData.every(
      (row) => isDataCell(row) || isDataRow(row)
    )
  )
